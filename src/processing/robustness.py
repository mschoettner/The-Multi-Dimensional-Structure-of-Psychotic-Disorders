...
def run_method(data, n_factors, iterations, sample_type, method, rotation="varimax"):
    """Runs the chosen dimensionality reduction method for a given number of iterations
    on different subsamples.

    Parameters
    ----------
    data : DataFrame
        DataFrame of the data to reduce, with variables as columns.
    n_factors : int
        Number of factors to extract
    iterations : int
        Number of subsamples to draw
    sample_type : str
        Sample type, can be "subjects" or "variables"
    method : str
        Dimensionality reduction method, can be "fa" for factor analysis, "pca", or "ica".

    Returns
    -------
    array
        Numpy array with loadings

    Raises
    ------
    NotImplementedError
        Raised when another method is given.
    """    
...
def reorder_factors(loadings):
    """Reorders the factors such that their order matches across subsamples.

    Parameters
    ----------
    loadings : array
        Numpy array of loadings

    Returns
    -------
    array
        Numpy array of reordered loadings
    """    
...
def calculate_abs_corr(loadings_reordered):
    """Calculates the absolute correlation between reordered loadings

    Parameters
    ----------
    loadings_reordered : array
        Numpy array of reordered loadings

    Returns
    -------
    array
        Numpy array of absolute correlations.
    """    
...
def robustness(data, n_factors, method, path, iterations=1000, sample_type="subjects", rotation="varimax"):
    """Calculates the robustness of a factor solution.
    ...
    -------
    array
        Numpy array with reordered loadings
    """    
    print(f"Running {method} with {iterations} iterations and rotation {rotation}")
    # abs_correlations = []
    for i in range(1, n_factors+1):
        # create output folder
        num1 = str(i).zfill(2)
        foldername = os.path.join("loadings", f"{method}_{n_factors}-components_{iterations}-iterations_rotation-{rotation}", f"{num1}-components")
        folderpath = os.path.join(path, foldername)
        os.makedirs(folderpath)
        print(f"Number of factors/components: {i}")
        # get loadings of run_method samples
        loadings = run_method(data, i, iterations, sample_type, method, rotation=rotation)
        # reordered loadings
...
if __name__ == "__main__":
    ...
    #datalad run -i data/03_processed/train_preprocessed.csv -o data/05_robustness/pca -m "run pca robustness" "python code/processing/robustness.py {inputs[0]} 11 pca data/05_robustness/pca/loadings"